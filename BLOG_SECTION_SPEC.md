# BlogSection — specifikacija i prompt za izvođača

## Kontekst

Ovo je sekcija **Blog & Vesti** na landing stranici AI Hype Academy (Next.js, React, Framer Motion, Tailwind). Prikazuje članke u grid-u sa kartama koje imaju **hero slideshow** (carousel slika), badge, naslov, intro, citat (opciono) i dugme za proširenje.

---

## Struktura komponente

- **Fajl:** `components/sections/BlogSection.tsx`
- **Importi:** `framer-motion`, `lucide-react`, `next/image`
- **Podaci:** niz `articles` sa objektima članaka
- **Komponente:** `ArticleCard` (pojedinačna kartica), glavni `BlogSection` export
...
```
ZADATAK: Održavanje / proširenje BlogSection komponente

KONTEKST:
- Next.js 16, React 19, TypeScript
- Framer Motion za animacije
- Tailwind CSS (globals.css ima .card, .section-container)
- Komponenta je u components/sections/BlogSection.tsx

STRUKTURA KARTICE (ArticleCard):
1. HERO SLIDESHOW (240px visine) umesto jedne slike — carousel/slideshow koji automatski prolazi kroz više slika. Opciono: dot indikatori ispod, pauza na hover, preferiraj prefers-reduced-motion (isključi auto-play ako korisnik preferira manje animacija). Gradijentni overlay (tamno na dnu, transparent na vrhu) ostaje preko slideshow-a.
2. Badge (npr. "VEST", "AI Alati") u gornjem levom uglu — dinamička boja po tipu članka
3. Opciono: overlay logo + naslov (za festival/event članke)
4. SADRŽAJ: datum, naslov, intro paragrafi
5. Opciono: citat u kutiji sa levim borderom u boji badge-a
6. Dugme "Pročitaj ceo članak" / "Prikaži manje" — expand/collapse punog teksta

INTERFEJS Article (ažuriran za slideshow):
- id, badge (icon, label, color, bg, border)
- heroImages: string[] — niz putanja slika za slideshow (npr. ["/img1.png", "/img2.png"]). Ako je jedna slika, i dalje radi (bez animacije ili fiksna).
- heroImage: string — backwards compatibility, ako postoji a heroImages nije, koristi samo ovu sliku
- overlayLogo (opciono), title, intro, quote (opciono), full, date

SLIDESHOW ZAHTEVI:
- Auto-advance svakih 4–5 sekundi
- Blaga fade ili crossfade animacija pri promeni slike
- Pause on hover (opciono)
- Respektuj prefers-reduced-motion: reduce — isključi auto-advance
- Dot indikatori na dnu hero oblasti (opciono, ako ima 2+ slike)

ŠTA MOŽEŠ RADITI:
- Implementirati hero slideshow (obavezno)
- Dodati nove članke u articles niz (sa heroImages za više slika)
- Promeniti stil kartica (border, shadow, hover)
- Prilagoditi animacije (delay, duration)
- Dodati link na članak (npr. /blog/[slug])
- Promeniti grid (1 col mobilno, 2 col desktop) — trenutno .blog-grid
- Ažurirati sadržaj postojećih članaka

FAJLOVI ZA SLIKE (u public/):
- blog-milivojka-thumb.png, blog-festival-logo.jpg, blog-theater.jpg
- blog-seedance-thumb.png, blog-seedance-hud.png, blog-seedance-control.png
```
...
## Pozadinski CSS (globals.css)

Kartice koriste klasu `.card`:
...
`.section-container` ograničava širinu i dodaje padding.
